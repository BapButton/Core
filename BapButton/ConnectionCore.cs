...
namespace BapButton
{
    public class TinkerConnectionCore : ITinkerController
    {
        private readonly ILogger<TinkerConnectionCore> _logger;
        ...
        private ISubscriber<StandardButtonCommandMessage> StandardButtonMessagePipe { get; set; }
        ...
        private ISubscriber<CustomImageMessage> CustomImageMessagePipe { get; set; }
        ...
        public TinkerConnectionCore(ILogger<TinkerConnectionCore> logger, IPublisher<NodeChangeMessage> nodeChangeSender, ISubscriber<StandardButtonCommandMessage> standardButtonMessagePipe, ISubscriber<RestartButtonMessage> restartButtonMessagePipe, ISubscriber<StatusButtonMessage> statusButtonMessagePipe, ISubscriber<CustomImageMessage> customImageMessagePipe, ISubscriber<TurnOffButtonMessage> turnOffButtonMessagePipe, IPublisher<ButtonPressedMessage> buttonPressSender, ISubscriber<InternalCustomImageMessage> internalCustomImage, GameHandler gameHandler)
        {
            _logger = logger;
            ...
            CustomImageMessagePipe = customImageMessagePipe;
        ...
        private async Task<bool> Initialize(string mqttServerIp = "", Action<MqttApplicationMessageReceivedEventArgs>? customCallback = null, string hostName = "")
        {
            ...
            StandardButtonMessagePipe.Subscribe(async (x) => await SendCommand(x.NodeId, x.StandardButtonMessage)).AddTo(bag);
            ...
            CustomImageMessagePipe.Subscribe(async (x) => await SendCustomImage(x.NodeId, x.CustomImage)).AddTo(bag);
        ...
        {
            ButtonLayout? currentLayout = GameHandler.CurrentButtonLayout;
        ...
        private async Task<bool> SendCommand(string nodeId, StandardButtonCommand standardButtonMessage)
        {
            if (string.IsNullOrEmpty(nodeId))
            {
                return await SendGeneralCommand(standardButtonMessage);
            }

            return await SendPrivate(nodeId, "command", standardButtonMessage);
        ...
        private async Task<bool> GetStatusFromAllButttons()
        {
            ...
            return await SendPublic("status", new ButtonDisplay());
        }

        private async Task<bool> RestartAllButtons()
        {
            ConnectedNodes = new ConcurrentDictionary<string, bool>();
            return await SendPublic("restart", new ButtonDisplay());
        }
        ...
        private async Task<bool> RestartAButton(string nodeId)
        {
            ...
            {
                return await SendPrivate(nodeId, "restart", new ButtonDisplay());
            }

        }

        private async Task<bool> SendGeneralCommand(StandardButtonCommand standardButtonMessage)
        {
            return await SendPublic("command", standardButtonMessage);
        }
        public async Task<bool> SendInternalCustomImage(string nodeId, InternalCustomImage internalCustomImage)
        {
            ...
            for (int r = 0; r < 8; r++)
            {
                for (int c = 0; c < 8; c++)
                {
                    data.Add(internalCustomImage.Image[r, c]);
                }
            }
            CustomImage customImage = new CustomImage();
            customImage.ImageData = data.ToArray();
            if (string.IsNullOrEmpty(nodeId))
            {
                return await SendCustomImage(customImage);
            }
            else
            {
                return await SendPrivate(nodeId, "customImage", customImage);
            }
        }

        public async Task<bool> SendCustomImage(string nodeId, CustomImage customImage)
        {
            if (string.IsNullOrEmpty(nodeId))
            {
                return await SendCustomImage(customImage);
        ...
        public async Task<bool> SendCustomImage(CustomImage customImage)
        {
            return await SendPublic("customImage", customImage);
        }

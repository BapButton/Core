...
public partial class HighScoreTable : ComponentBase, IDisposable
{
    ...
    [Inject]
    public KeyboardHandler KeyboardHandler { get; set; } = default!;
    [Inject]
    ...
    protected override async Task OnInitializedAsync()
    {
        ...
        else
        {
            ...
            {
                var currentKeyboard = KeyboardHandler.SetNewKeyboard<DefaultKeyboard>(true);
                currentKeyboard.SetupKeyboard(KeyboardType.CapitalLetters, showX: false, playKeyPressSound: true);
                currentKeyboard.ShowKeyboard();
            }
        }
    ...
    public async Task SaveNewHighScore()
    {
        if (NewUserName.Length > 2)
        {
            ...
            if (scoreToSave != null)
            {
                ...
                KeyboardHandler.KeyboardController.Disable(true);
                await InvokeAsync(() =>
                {
                    StateHasChanged();
                });
    ...
    public void Dispose()
    {
        KeyboardHandler.KeyboardController.Dispose();

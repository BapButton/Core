...
namespace BapWeb.Games
{
    ...
    public class ExplodingBapGame : ITinkerGame
    {
        ...
        public ExplodingBapGame(ILogger<ExplodingBapGame> logger, MessageSender messageSender, GameHandler gameHandler, ISubscriber<ButtonPressedMessage> buttonPressedPipe)
        {
            Logger = logger;
        ...
        public async Task<bool> Start()
        {
            ...
            foreach (var row in GameHandler.CurrentButtonLayout.ButtonPositions.GroupBy(t => t.RowId).OrderBy(t => t.Key))
            {
        ...
        {
            List<InternalCustomImageMessage> images = new();
                ...
                if (!row.ReversedDirection)
                {
                    ...
                    if (row.ImageTrackingArray[lastItemInArray] > -1)
                    {
                        ...
                        if (endOfImage == lastItemInArray)
                        {
                            ...
                            {
                                row.ImageTrackingArray[lastItemInArray] = TinkerButtonGameMethods.GetRandomInt(0, PossibleImages.Count, currentImage, 50);
                ...
                else
                {
                    ...
                    if (row.ImageTrackingArray[0] > -1)
                    {
                        ...
                        if (endOfImage == 0)
                        {
                            ...
                            {
                                row.ImageTrackingArray[0] = TinkerButtonGameMethods.GetRandomInt(0, PossibleImages.Count, currentImage, 50);
            ...
            {
                MsgSender.SendInternalCustomImage(image);
            }
        }

        private List<InternalCustomImageMessage> GenerateImagesForRow(ExplodingBapRow row)
        {
            int fullFrameLength = row.ImageTrackingArray.Length;
            List<InternalCustomImageMessage> images = new List<InternalCustomImageMessage>();
            ...
            {
                images.Add(new(bigMatrix.ExtractMatrix(0, (i * 8)), row.NodeIdsOrderedLeftToRight[i]));
            }
            return images;
        }
        ...
        {
            List<InternalCustomImageMessage> allCurrentImages = new List<InternalCustomImageMessage>();
            foreach (var row in Rows)
            {
            ...
            if (currentExplosionTracker != null)
            {
                ...
                foreach (var nodeId in currentExplosionTracker.ActiveNodeIds)
                {
                    ...
                    if (currentMessage != null)
                    {
                        ...
                        currentMessage.CustomImage.Image.MergeMatrices(explosionOverLay, overLayRow, overLayColumn);
                        MsgSender.SendInternalCustomImage(currentMessage);

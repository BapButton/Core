...
namespace BapWeb.Games
{
    ...
    public class SwordBonusGame : ReactionGameBase
    {
        ...
        public SwordBonusGame(ILogger<SwordBonusGame> logger, IPublisher<InternalSimpleGameUpdates> internalGameUpdates, ISubscriber<ButtonPressedMessage> buttonPressed, MessageSender messageSender) : base(buttonPressed, messageSender)
        {
        ...
        public override async Task<bool> NextCommand()
        {
            ...
            if (IsGameRunning)
            {
                if (TinkerButtonGameMethods.ShouldWePerformARandomAction(6))
                {

                    var crownImage = new ButtonDisplay(0, 0, 255, Patterns.NoPattern, (int)SBGCustomImages.Crown + 1, CrownDelay);
                    lastNodeId = TinkerButtonGameMethods.GetRandomNodeId(buttons, lastNodeId, 2);
